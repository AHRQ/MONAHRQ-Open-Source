/*
 *      Name:           spQualityInitializeHCBinary
 *      Version:        1.0
 *      Last Updated:   7/14/14
 *      Used In:        QualityReportGenerator.cs
 *      Description:    Used to prep all of the HC Binary measures. These are the patient experience measures.
 */

IF EXISTS (
	SELECT * 
    FROM INFORMATION_SCHEMA.ROUTINES 
	WHERE SPECIFIC_SCHEMA = N'dbo' AND SPECIFIC_NAME = N'spQualityInitializeHCBinary' 
)
	DROP PROCEDURE dbo.spQualityInitializeHCBinary
GO

CREATE PROCEDURE [dbo].[spQualityInitializeHCBinary]
...
EXEC dbo.spQualityGetHospitalCompareMeasures @HospitalCompareDataset, @Hospitals, @Measures, 'Binary', @RegionType
...
INSERT INTO #Temp_Quality_Data (MeasureID, HospitalID, CountyID, RegionID, ZipCode, HospitalType, MeasureName, MeasureType,
    ObservedDenominator, RiskAdjustedRate, Threshold, HigherScoresAreBetter,
...
SELECT MeasureID, HospitalID, CountyID, RegionID, ZipCode, HospitalType, MeasureName, MeasureType,
    ISNULL([Sample], -1), ISNULL(Rate, -1), ISNULL(CategoryCode, -1), HigherScoresAreBetter,
...
WITH
    ...
    (
        ...
        WHERE t.Name = 'Hospital Compare Data'
            AND Measures.MeasureType IN ('Binary')
            AND Measures.Id IN (
                SELECT Id
                FROM @Measures
            )
...
SET NatFilled = dbo.fnQualityBarChartPercentage(#Temp_Quality_Barcharts.MinNatBar, #Temp_Quality_Barcharts.MaxNatBar, RiskAdjustedRate),
    ...
    Col2 = CONVERT(VARCHAR, CONVERT(DECIMAL(8,0),100 - RiskAdjustedRate)),
    Col4 = CONVERT(VARCHAR, ObservedDenominator)
FROM #Temp_Quality_Data
...
begin
	...
	UPDATE #Temp_Quality_Data
	SET NatRating = 0, NatFilled = -1, PeerRating = 0, PeerFilled = -1, RateAndCI = '-', Col1 = '-', Col2 = '-'
	WHERE RiskAdjustedRate < 0
			OR RiskAdjustedRate IS NULL

	UPDATE #Temp_Quality_Data
	SET NatRating = 0, NatFilled = -1, PeerRating = 0, PeerFilled = -1, RateAndCI = 'c', Col1 = 'c', Col2 = 'c'
	WHERE Col2 <> '-'
...
SELECT DISTINCT
    ...
    -- Columns descriptions
    'Patients responding &quot;Yes&quot;' AS ColDesc1,
    'Patients responding &quot;No&quot;' AS ColDesc2,
    'Surveys completed' AS ColDesc3,
    'Response rate' AS ColDesc4,
