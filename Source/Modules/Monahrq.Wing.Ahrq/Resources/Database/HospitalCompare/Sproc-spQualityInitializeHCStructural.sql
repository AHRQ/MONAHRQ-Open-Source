/*
 *      Name:           spQualityInitializeHCStructural
 *      Version:        1.0
 *      Last Updated:   7/15/14
 *      Used In:        QualityReportGenerator.cs
 *      Description:    Used to prep all of the HC Structural measures. These are the patient experience measures.
 */

IF EXISTS (
	SELECT * 
    FROM INFORMATION_SCHEMA.ROUTINES 
	WHERE SPECIFIC_SCHEMA = N'dbo' AND SPECIFIC_NAME = N'spQualityInitializeHCStructural' 
)
	DROP PROCEDURE dbo.spQualityInitializeHCStructural
GO

CREATE PROCEDURE [dbo].[spQualityInitializeHCStructural]
...
EXEC dbo.spQualityGetHospitalCompareMeasures @HospitalCompareDataset, @Hospitals, @Measures, 'Structural', @RegionType
...
WITH
    ...
    (
        ...
        WHERE t.Name = 'Hospital Compare Data'
            AND Measures.MeasureType IN ('Structural')
            AND Measures.Id IN (
                SELECT Id
                FROM @Measures
            )
...
SELECT DISTINCT
    ...
    -- Columns descriptions
    'Sample Value' AS ColDesc1,
    '' AS ColDesc2, '' AS ColDesc3, '' AS ColDesc4, '' AS ColDesc5,
...
FROM			dbo.Measures
    left JOIN	MeasureTopics ON dbo.Measures.Id = MeasureTopics.MeasureID
