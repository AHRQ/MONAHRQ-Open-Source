/*
 *      Name:           spQualityInitializeHCYNM
 *      Version:        1.0
 *      Last Updated:   7/15/14
 *      Used In:        QualityReportGenerator.cs
 *      Description:    Used to prep all of the HC YNM measures. These are the patient experience measures.
 */

IF EXISTS (
	SELECT * 
    FROM INFORMATION_SCHEMA.ROUTINES 
	WHERE SPECIFIC_SCHEMA = N'dbo' AND SPECIFIC_NAME = N'spQualityInitializeHCYNM' 
)
	DROP PROCEDURE dbo.spQualityInitializeHCYNM
GO

CREATE PROCEDURE [dbo].[spQualityInitializeHCYNM]
...
EXEC dbo.spQualityGetHospitalCompareMeasures @HospitalCompareDataset, @Hospitals, @Measures, 'YNM', @RegionType
...
WITH
    ...
    (
        ...
        WHERE t.Name = 'Hospital Compare Data'
            AND Measures.MeasureType IN ('YNM')
            AND Measures.Id IN (
                SELECT Id
                FROM @Measures
            )
...
-- Prep Yes-No-Maybe Measures for detail reporting H_RECMND_DY/ H_RECMND_PY/ H_RECMND_DN
-- get the Definitely-No answers paired up with Definitely-Yes in [ObservedRate] column
UPDATE #Temp_Quality_Data
...
WHERE HC.MeasureCode = #Temp_Quality_Data.MeasureName + '-DN'
...
-- get the Possibly-Yes answers paired up with Definitely-Yes in [ExpectedRate] column
UPDATE #Temp_Quality_Data
SET #Temp_Quality_Data.ExpectedRate = HC.Rate
FROM dbo.Targets_HospitalCompareTargets HC
    JOIN dbo.Hospitals Hosp ON HC.CMSProviderID = Hosp.CmsProviderID
WHERE HC.MeasureCode = #Temp_Quality_Data.MeasureName + '-PY'
    AND #Temp_Quality_Data.HospitalID = Hosp.Id
    AND MeasureType = 'YNM'
    AND HC.Dataset_id IN (
        SELECT Id
...
SELECT DISTINCT
    ...
    -- Columns descriptions
    'Patient responding &quot;Definitely Yes&quot;' AS ColDesc1,
    'Patient responding &quot;Possibly Yes&quot;' AS ColDesc2,
    'Patient responding &quot;Definitely No&quot;' AS ColDesc3,
