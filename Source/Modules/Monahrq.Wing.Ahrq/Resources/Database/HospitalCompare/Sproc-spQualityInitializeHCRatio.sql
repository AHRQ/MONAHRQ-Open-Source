/*
 *      Name:           spQualityInitializeHCRatio
 *      Version:        1.0
 *      Last Updated:   8/1/14
 *      Used In:        QualityReportGenerator.cs
 *      Description:    Used to prep all of the HC Ratio measures.
 */

IF EXISTS (
	SELECT * 
    FROM INFORMATION_SCHEMA.ROUTINES 
	WHERE SPECIFIC_SCHEMA = N'dbo' AND SPECIFIC_NAME = N'spQualityInitializeHCRatio' 
)
	DROP PROCEDURE dbo.spQualityInitializeHCRatio
GO

CREATE PROCEDURE [dbo].[spQualityInitializeHCRatio]
...
(
    ...
    RiskAdjustedRate decimal(19,7) NULL,                 -- Col1
...
EXEC dbo.spQualityGetHospitalCompareMeasures @HospitalCompareDataset, @Hospitals, @Measures, 'Ratio', @RegionType
...
INSERT INTO #Temp_Quality_Data (MeasureID, HospitalID, CountyID, RegionID, ZipCode, HospitalType, MeasureName,
	RiskAdjustedRate, RiskAdjCILow, RiskAdjCIHigh, NatBenchmarkRate)
SELECT MeasureID, HospitalID, CountyID, RegionID, ZipCode, HospitalType, MeasureName,
    ISNULL(Rate, -1), ISNULL([Lower], -1), ISNULL([Upper], -1), NationalBenchmark
...
/*
-- TODO: Add to import.
-- NOTE: 1.0 is in the HC database but was not imported historically for some reason, so it was hardcoded here.
UPDATE #Temp_Quality_Data
SET NatBenchmarkRate = 1.0
FROM #Temp_Quality_Data
WHERE NatBenchmarkRate <= 0;
*/

-- * Can be used once data is imported in base data.
...
WHERE (#Temp_Quality_Data.NatBenchmarkRate <= 0 OR #Temp_Quality_Data.NatBenchmarkRate IS NULL)
...
-- Check for the lower is better measures.
UPDATE #Temp_Quality_Data
SET NatRating =
        CASE
    ...
    PeerRating =
        ...
        END
FROM #Temp_Quality_Data
	LEFT JOIN #Temp_Quality_Measures ON #Temp_Quality_Data.MeasureName = #Temp_Quality_Measures.MeasureName
WHERE #Temp_Quality_Measures.HigherScoresAreBetter = 0;

-- Check for the higher is better measures.
UPDATE #Temp_Quality_Data
SET NatRating =
        CASE
            WHEN RiskAdjustedRate < 0 THEN 0
...
FROM #Temp_Quality_Data
	LEFT JOIN #Temp_Quality_Measures ON #Temp_Quality_Data.MeasureName = #Temp_Quality_Measures.MeasureName
WHERE #Temp_Quality_Measures.HigherScoresAreBetter = 1;
...
SET NatFilled = dbo.fnQualityBarChartPercentage(#Temp_Quality_Barcharts.MinNatBar, #Temp_Quality_Barcharts.MaxNatBar, RiskAdjustedRate),
    ...
    RateAndCI = CONVERT(VARCHAR, CONVERT(DECIMAL(8,1),RiskAdjustedRate)) + ' (' + CONVERT(VARCHAR, CONVERT(DECIMAL(8,1),RiskAdjCILow)) + ' - ' + CONVERT(VARCHAR, CONVERT(DECIMAL(8,1),RiskAdjCIHigh)) + ')',
    Col1 = CONVERT(VARCHAR, CONVERT(DECIMAL(8,4),RiskAdjustedRate)),
	Col2 = CONVERT(VARCHAR, CONVERT(DECIMAL(8,1),RiskAdjCILow)),
	Col3 = CONVERT(VARCHAR, CONVERT(DECIMAL(8,1),RiskAdjCIHigh))
FROM #Temp_Quality_Data
...
SELECT DISTINCT
    ...
    -- Columns descriptions
    'Score' AS ColDesc1,
    'Lower-bound CI' AS ColDesc2,
	'Upper-bound CI' AS ColDesc3,
    ...
    'Measure Ratio' AS StatisticsAvailable,
