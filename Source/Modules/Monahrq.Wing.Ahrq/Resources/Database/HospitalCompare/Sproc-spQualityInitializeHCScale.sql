/*
 *      Name:           spQualityInitializeHCScale
 *      Version:        1.0
 *      Last Updated:   7/15/14
 *      Used In:        QualityReportGenerator.cs
 *      Description:    Used to prep all of the HC Scale measures. These are the patient experience measures.
 */

IF EXISTS (
	SELECT * 
    FROM INFORMATION_SCHEMA.ROUTINES 
	WHERE SPECIFIC_SCHEMA = N'dbo' AND SPECIFIC_NAME = N'spQualityInitializeHCScale' 
)
	DROP PROCEDURE dbo.spQualityInitializeHCScale
GO

CREATE PROCEDURE [dbo].[spQualityInitializeHCScale]
...
EXEC dbo.spQualityGetHospitalCompareMeasures @HospitalCompareDataset, @Hospitals, @Measures, 'Scale', @RegionType
...
WITH
    ...
    (
        ...
        WHERE t.Name = 'Hospital Compare Data'
            AND Measures.MeasureType IN ('Scale')
            AND Measures.Id IN (
                SELECT Id
                FROM @Measures
            )
...
FROM SecondBest
    JOIN #Temp_Quality_Data ON SecondBest.MeasureID = #Temp_Quality_Data.MeasureID
...
-- Prep Scale Measures for detail reporting
-- get the 0-6 answers paired up with 9-10 in observed-rate column
UPDATE #Temp_Quality_Data
...
WHERE HC.MeasureCode = #Temp_Quality_Data.MeasureName + '-0-6'
    AND #Temp_Quality_Data.HospitalID = Hosp.Id
    AND #Temp_Quality_Data.MeasureType = 'Scale'
    AND HC.Dataset_id IN (
        SELECT Id
        FROM @HospitalCompareDataset
    );
...
-- get the 7-8 answers paired up with 9-10 in expected-rate column
UPDATE #Temp_Quality_Data
SET #Temp_Quality_Data.ExpectedRate = HC.Rate
FROM dbo.Targets_HospitalCompareTargets HC
    JOIN dbo.Hospitals Hosp ON HC.CMSProviderID = Hosp.CmsProviderID
WHERE HC.MeasureCode = #Temp_Quality_Data.MeasureName + '-7-8'
    AND #Temp_Quality_Data.HospitalID = Hosp.Id
    AND #Temp_Quality_Data.MeasureType = 'Scale'
    AND HC.Dataset_id IN (
        SELECT Id
...
SELECT DISTINCT
    ...
    -- Columns descriptions
    'Patient rating of 9 or 10' AS ColDesc1,
    'Patient rating of 7 or 8' AS ColDesc2,
    'Patient rating of 6 or lower' AS ColDesc3,
