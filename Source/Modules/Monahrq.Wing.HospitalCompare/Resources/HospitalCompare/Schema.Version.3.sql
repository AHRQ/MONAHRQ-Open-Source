...
true,[Denominator] ) AS [Sample],
switch( 
trim(RCD.[Lower Estimate]) ='Not Available' ,-1,
true, [Lower Estimate] ) AS [Lower], 
switch( 
trim(RCD.[Higher Estimate])  ='Not Available' ,-1,
true,[Higher Estimate]  ) AS [Upper],
null AS [Note],
null AS [BenchmarkID],
null AS [Footnote_Id]
...
AS  Rate,
 switch( 
 trim([HQI_HOSP_TimelyEffectiveCare].[Sample]) ='Not Available' ,-1,
true, [HQI_HOSP_TimelyEffectiveCare].[Sample] ) AS Sample,
-1 as [Lower],
-1 as [Upper]
...
sum(
switch(
[Measure ID]='HAI_3_SIR' and trim([Score])='Not Available',-1,
[Measure ID]='HAI_3_SIR',val([Score]),
true,0
)
) as Rate,
sum(
switch(
[Measure ID]='HAI_3_CI_LOWER' and (trim([Score])='Not Available' or [Score]='-') ,-1,
[Measure ID]='HAI_3_CI_LOWER',val([Score]),
true,0
)
) as [Lower],
sum(
switch(
[Measure ID]='HAI_3_CI_UPPER' and trim([Score])='Not Available',-1,
[Measure ID]='HAI_3_CI_UPPER',val([Score]),
true,0
)
) as [Upper]
from
...
sum(
switch(
[Measure ID]='HAI_4_SIR' and trim([Score])='Not Available',-1,
[Measure ID]='HAI_4_SIR',val([Score]),
true,0
)
) as Rate,
sum(
switch(
[Measure ID]='HAI_4_CI_LOWER' and (trim([Score])='Not Available' or [Score]='-') ,-1,
[Measure ID]='HAI_4_CI_LOWER',val([Score]),
true,0
)
) as [Lower],
sum(
switch(
[Measure ID]='HAI_4_CI_UPPER' and trim([Score])='Not Available',-1,
[Measure ID]='HAI_4_CI_UPPER',val([Score]),
true,0
)
) as [Upper]
from
...
-1 as [Upper],
'US' AS BenchmarkID
                        FROM [HQI_NATIONAL_HAI] WHERE RIGHT([Measure ID], 4) = '_SIR' 
...
switch( 
trim([HQI_HOSP_TimelyEffectiveCare].Sample)  ='Not Available' ,-1,
true,[HQI_HOSP_TimelyEffectiveCare].Sample  )
AS [Sample],
-1 as [Lower],
-1 as [Upper] ,
0 AS ConditionCode,
...
select
[HQI_HOSP].[Provider ID] as CMSProviderID,
switch( 
trim([HQI_HOSP].[Hospital overall rating]) is null, 0,
trim([HQI_HOSP].[Hospital overall rating]) = 'Not Available', 0,
true,[HQI_HOSP].[Hospital overall rating]) as [Sample],
