...
SELECT	RCD.[Provider ID] AS [CMSProviderID]
	...
	,	null AS [BenchmarkID]
	,	null AS [Footnote_Id]
FROM [Readmissions and Deaths - Hospital.csv] RCD
...
FROM [Timely and Effective Care - State.csv]
...
-1 as [Upper],
'US' AS BenchmarkID
                        FROM [Readmissions and Deaths - National.csv]
...
select			RCD.[Provider ID] AS [CMSProviderID]
			...
			,	null AS [BenchmarkID]
			,	null AS [Footnote_Id]
FROM			[Complications - Hospital.csv] RCD
...
select			-1 AS ConditionCode
			...
			,	-1 as [Upper]
			,	'US' AS BenchmarkID
from			[Complications - National.csv]
...
SELECT	REPLACE ( [Provider ID],'''','') AS [CMSProviderID]
	...
	,	-1 as [Lower]
	,	-1 as [Upper]
FROM [Timely and Effective Care - Hospital.csv] 
...
SELECT 
				...
				-1 as [Upper],
				'US'  AS BenchmarkID
FROM			[Timely and Effective Care - National.csv] 
...
-1 as [Lower],
-1 as [Upper]
                        FROM [Outpatient Imaging Efficiency - Hospital.csv] 
...
-1 as [Upper],
State AS BenchmarkID
                        FROM [Outpatient Imaging Efficiency - State.csv] 
...
-1 as [Upper],
'US' AS BenchmarkID
                        FROM [Outpatient Imaging Efficiency - National.csv] 
...
-1 as [Upper],
'US' AS BenchmarkID
FROM [Healthcare Associated Infections - National.csv] 
...
from [Hospital General Information.csv]
...
SELECT			[Provider ID] AS [CMSProviderID]
			...
			,	0 as ConditionCode
			,	0 as [CategoryCode]
FROM			[Structural Measures - Hospital.csv]
