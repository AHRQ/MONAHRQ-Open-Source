...
{
    /// <summary>
    /// Class for file process.
    /// </summary>
    /// <seealso cref="Monahrq.Theme.Controls.Wizard.Models.WizardStepViewModelBase{Monahrq.Wing.MedicareProviderCharge.Models.WizardContext}" />
    ...
    public class ProcessFileViewModel : WizardStepViewModelBase<WizardContext>
    {
        ...
        protected DataTable MedicareProviderDataTable;
        ...
        {
            Done = false;
            MedicareProviderDataTable = new DataTable("MedicareProviderChargeTarget")
            {
                TableName = typeof(MedicareProviderChargeTarget).EntityTableName()
            };
        }
...
// ReSharper restore UnusedParameter.Local
        ...
        {
            var targetTableName = string.Format("dbo.{0}", typeof (MedicareProviderChargeTarget).EntityTableName());
                    ...
                    using (var inputFile = new StreamReader(fs))
                    {
                        ...
                        while ((inputLine = inputFile.ReadLine()) != null)
                        {
                            ...
                            if (inputLine.Replace(" ", null).ToUpperInvariant().StartsWith(_headerLine.Replace(" ", null).ToUpperInvariant()))
                            {
                                ...
                                MedicareProviderDataTable.Columns.Clear();
                                MedicareProviderDataTable.Columns.AddRange(headerColumns);
						...
						DataContextObject.DatasetItem.ReportingYear = DataContextObject.Year;
                        ...
                        while ((inputLine = inputFile.ReadLine()) != null)
                        {
                            try
                            {
                                ...
                                {
                                    BulkInsert(MedicareProviderDataTable);
                                    MedicareProviderDataTable.Rows.Clear();
                                    lineCount = 0;
                                }
                            }
                            catch (Exception e)
