...
namespace Monahrq.Wing.NursingHomeCompare.NHCAHPS.ViewModel
{
    ...
    public class ProcessFileViewModel : WizardStepViewModelBase<WizardContext>
    {
        ...
        {
            Done = false;
            NHCAHPSSurveyDataTable = new DataTable("NHCAHPSSurveyTarget")
            {
                TableName = typeof(NHCAHPSSurveyTarget).EntityTableName()
            };
        }
...
// ReSharper restore UnusedParameter.Local
        ...
        {
            var targetTableName = string.Format("dbo.{0}", typeof (NHCAHPSSurveyTarget).EntityTableName());
        ...
        /// <returns></returns>
        protected bool Any<T>(string providerId , string nhId) where T: NHCAHPSSurveyTarget
        {
            return _session.Query<T>()
                           .Any(m => m.ProviderId.ToLower() == providerId.ToLower() && m.NHId.ToLower() == nhId.ToLower());
        }
