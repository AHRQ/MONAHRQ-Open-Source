...
{
	/// <summary>
	/// Model used for progressing through the NHCompare Import screens.
	/// </summary>
		...
		private void DeletePreviousImport()
        {
                    ...
                    ServiceLocator.Current.GetInstance<IDomainSessionFactoryProvider>().SessionFactory.OpenSession())
            {
                ...
                using (var transaction = session.BeginTransaction())
                {
                    ...
                    session.CreateSQLQuery(string.Format("TRUNCATE TABLE {0}", typeof(NHDeficiencyTarget).EntityTableName()))
                           .SetTimeout(300)
                           .ExecuteUpdate();
