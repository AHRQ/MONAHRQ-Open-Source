...
IF EXISTS (
	...
	WHERE SPECIFIC_SCHEMA = N'dbo' AND SPECIFIC_NAME = N'spInitNursingHomeReport_V2' 
)
   DROP PROCEDURE dbo.spInitNursingHomeReport_V2
GO

if exists(select * from sys.types where name = 'InitNursingHomeReportstringIdentifiers_V2')
    drop type InitNursingHomeReportstringIdentifiers_V2
go

create type InitNursingHomeReportstringIdentifiers_V2 AS TABLE 
( id nvarchar(max));
go
...
CREATE PROCEDURE dbo.spInitNursingHomeReport_V2
	@NursingHomes InitNursingHomeReportstringIdentifiers_V2 readonly,
	@DatasetID int
AS
BEGIN
    SET NOCOUNT ON;
...
set QMtotal_Raw = 
	(QM401_percentile + QM402_percentile + QM403_percentile + QM406_percentile + QM407_percentile + QM409_percentile + QM410_percentile + QM419_percentile)*11/8
