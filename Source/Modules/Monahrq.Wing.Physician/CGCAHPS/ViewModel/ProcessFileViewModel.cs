...
{
	/// <summary>
	/// Handles user selection of importing CGCAHPS data.
	/// </summary>
	/// <seealso cref="Monahrq.Theme.Controls.Wizard.Models.WizardStepViewModelBase{Monahrq.Wing.Physician.CGCAHPS.Model.WizardContext}" />
    ...
    public class ProcessFileViewModel : WizardStepViewModelBase<WizardContext>
    {
		...
		/// <summary>
		/// The nhcahps survey data table
		/// </summary>
		protected DataTable NHCAHPSSurveyDataTable;
        ...
        {
            Done = false;
            NHCAHPSSurveyDataTable = new DataTable("CGCAHPSSurveyTarget")
            {
                TableName = typeof(CGCAHPSSurveyTarget).EntityTableName()
            };
        }
        ...
        {
            var targetTableName = string.Format("dbo.{0}", typeof (CGCAHPSSurveyTarget).EntityTableName());
		...
		/// <param name="cgId">The cg identifier.</param>
		/// <returns></returns>
		protected bool Any<T>(string providerId , string cgId) where T: CGCAHPSSurveyTarget
        {
            return _session.Query<T>()
                           .Any(m => m.CGPracticeId.ToLower() == providerId.ToLower() && m.CGId.ToLower() == cgId.ToLower());
        }
                                ...
                                var headerColumns = new[]
                                    {
                                        ...
                                        new DataColumn(DB_COLUMN_DRG_ID, typeof(int)){ AllowDBNull = false}, 
                                        new DataColumn(DB_COLUMN_DRG, typeof (string)) { AllowDBNull = false },
                                        new DataColumn(DB_COLUMN_PROVIDER_ID, typeof(string)) { AllowDBNull = false },
                                        //new DataColumn(DB_COLUMN_PROVIDER_STATE_ABBREV, typeof(string)) { AllowDBNull = true },
                                        new DataColumn(DB_COLUMN_TOTAL_DISCHARGES, typeof(int)) { AllowDBNull = true },
                                        new DataColumn(DB_COLUMN_AVERAGE_COVERED_CHARGES, typeof (double)) { AllowDBNull = true },
                                        new DataColumn(DB_COLUMN_AVERAGE_TOTAL_PAYMENTS, typeof (double)) { AllowDBNull = true },
