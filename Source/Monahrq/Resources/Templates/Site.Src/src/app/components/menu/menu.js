...
(function () {
    ...
    function menu($compile, SortSvc, $window) {
        var childTpl;
        ...
        function init() {
            childTpl = _.template('<li data-ng-class="{active: navActive([<%=activeName%>], [<%=ignoreName%>])}">' +
              ...
              '<span><%=label%></span>' +
              '<span class="sr-hidden" data-ng-if="navActive([<%=activeName%>], [<%=ignoreName%>])"> is active</span>' +
              '</a>');
        }
        ...
        function link(scope, elem, attrs) {
            var $root = angular.element('<nav role="navigation" aria-label="Site Navigation"><ul></ul></nav>');
