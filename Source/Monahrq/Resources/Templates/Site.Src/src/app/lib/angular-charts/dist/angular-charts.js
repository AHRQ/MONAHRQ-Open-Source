   ...
   */
     ...
     */
      function setContainers() {
        ...
        var actitle = getChildrenByClassname(childrens, 'ac-title');
        height -= actitle ? actitle[0].clientHeight : 0;
      }
      /**
     ...
     */
      function lineChart() {
        ...
        if (config.xDemarcation) {
          var demarcTick = d3.selectAll('.x.axis g.tick')
            .filter(function(d){ return d == config.xDemarcation; });

          var demarcLine = demarcTick.append('line')
              .attr('y2', -height)
              .style('stroke', 'red');

          if (config.xDemarcationLabelLeft) {
            demarcTick.append('text').attr('dx', -5).attr('dy', -height + 25)
              .attr('fill', 'red')
              .attr('text-anchor', 'end')
              .text(function (d) {
                return config.xDemarcationLabelLeft;
              });
          }
          if (config.xDemarcationLabelRight) {
            demarcTick.append('text').attr('dx', 5).attr('dy', -height + 25)
              .attr('fill', 'red')
              .attr('text-anchor', 'start')
              .text(function (d) {
                return config.xDemarcationLabelRight;
              });
          }
        }
