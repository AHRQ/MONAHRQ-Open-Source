...
(function() {
  ...
  angular.module('monahrq.products.professional.physicians')
    .controller('PhysiciansProfileCtrl', PhysiciansProfileCtrl);

  PhysiciansProfileCtrl.$inject = ['$scope', '$state', '$stateParams', '$q', 'ResourceSvc', 'PhysicianAPISvc',
    'PhysicianReportLoaderSvc', 'PhysicianDedupeSvc', 'ReportConfigSvc', 'ModalLegendSvc', 'hospitals', 'PhysicianCAHPSSvc',
    'ModalTopicCategorySvc', 'ModalTopicSvc', 'ModalMeasureSvc', 'ModalGenericSvc', 'ScrollToElSvc'];
  function PhysiciansProfileCtrl($scope, $state, $stateParams, $q, ResourceSvc, PhysicianAPISvc,
                                 PhysicianReportLoaderSvc, PhysicianDedupeSvc, ReportConfigSvc, ModalLegendSvc, hospitals, PhysicianCAHPSSvc,
                                 ModalTopicCategorySvc, ModalTopicSvc, ModalMeasureSvc, ModalGenericSvc, ScrollToElSvc) {
    var _config;
    ...
    function init() {
      reportConfig = ReportConfigSvc.configForReport($scope.reportId);
    ...
    function getChildConfig(report, activeTab) {
      ...
      if (_.isObject(report.ChildConfig) && activeTab) {
        ...
        else if (activeTab === 'ratings' && report.ChildConfig.MedicalPractice.HelpOverride) {
    ...
    function setupReportHeaderFooter() {
      ...
      var childConfig = getChildConfig(report, $scope.tab);
    ...
    function processData(physicians) {
      ...
      genInfo.erx = (genInfo.erx == 'Y' ? 'Yes' : 'No');
      genInfo.pqrs = (genInfo.pqrs == 'Y' ? 'Yes' : 'No');
      genInfo.ehr = (genInfo.ehr == 'Y' ? 'Yes' : 'No');
      $scope.practices = [];
    ...
    function selectPractice(id) {
      ...
      $scope.goToTab('ratings');
      ScrollToElSvc.scrollToEl('.page--physicians');
    }
