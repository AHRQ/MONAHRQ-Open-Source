/*
 * CREDIT -  Originally adapted from Inflector.Net (http://andrewpeters.net/inflectornet/)
 */
...
namespace Monahrq.Sdk.Utilities
{
    ///<summary>
    ...
    public static class Inflector
    {
        ...
        private class Rule
        {
            ...
            {
                _regex = new System.Text.RegularExpressions.Regex(pattern, RegexOptions.IgnoreCase);
        ...
        {
            _uncountables.Add(word.ToLower(CultureInfo.InvariantCulture));
        }
        ...
        {
            return System.Text.RegularExpressions.Regex.Replace(Humanize(Underscore(word)), @"\b([a-z])",
                                 match => match.Captures[0].Value.ToUpper(CultureInfo.InvariantCulture));
        }

        public static string Titleize2(string word)
        {
            return
                word = string.Join(
                    string.Empty,
                    word.Select((x, i) => (
                                           char.IsUpper(x) && i > 0 &&
                                           (char.IsLower(word[i - 1]) || (i < word.Count() - 1 && char.IsLower(word[i + 1])))
                                       )
                                           ? " " + x
                                           : x.ToString()));
        }
        ...
        {
            return Capitalize(System.Text.RegularExpressions.Regex.Replace(lowercaseAndUnderscoredWord, @"_", " "));
        }
        ...
        {
            return System.Text.RegularExpressions.Regex.Replace(lowercaseAndUnderscoredWord, "(?:^|_)(.)",
        ...
        public static string Underscore(string pascalCasedWord)
        {
            return System.Text.RegularExpressions.Regex.Replace(
                System.Text.RegularExpressions.Regex.Replace(
                    System.Text.RegularExpressions.Regex.Replace(pascalCasedWord, @"([A-Z]+)([A-Z][a-z])", "$1_$2"), @"([a-z\d])([A-Z])",
                    "$1_$2"), @"[-\s]", "_").ToLower(CultureInfo.InvariantCulture);
        }
		...
		public static string ProperCase(string word,string otherDelimiters="")
		{
			if (string.IsNullOrWhiteSpace(word))
				return word;

			var delimiters = new [] { ' ', '\t', ((char)160) }.ToList();
			if (otherDelimiters != null)
				delimiters.AddRange(otherDelimiters.ToCharArray());

		    var parts = word.SplitAndKeep( delimiters.ToArray() );

		    return parts.Aggregate("", (current, part) => current + Capitalize(part));
		}
        ...
        {
            return word.Substring(0, 1).ToLower(CultureInfo.InvariantCulture) + word.Substring(1);
        }
